...
def collide(np.ndarray[np.float_t, ndim=2] v,
    ...
    cdef double v_1_dot_r_sep, v_2_dot_r_sep, R_c_sq = square(R_c), R_sep_sq
...
def vicsek(np.ndarray[np.float_t, ndim=2] v, list interacts):
    cdef unsigned int i_1, i_2, i_dim
...
def vicsek_fort(np.ndarray[np.float_t, ndim=2] v,
        np.ndarray inters,
        np.ndarray intersi):
    cdef unsigned int i_1, i_i_2, i_2, i_dim
    cdef np.ndarray[np.float_t, ndim = 2] v_vic = v.copy()
