...
class Arrows_plot():
    ...
    def plot_update(self, arrows, box, t):
        P.imshow(np.mean(self.density_map, axis=0).T, 
...
#            P.imshow(box.walls.T, cmap=matplotlib.cm.gray,
#                     interpolation='nearest',
#                     extent=self.extent, origin='lower')

#            p = P.imshow(overlay.T, cmap=matplotlib.cm.summer,
#                         interpolation='nearest',
#                         extent=self.extent, origin='lower', alpha=0.8)
            #!!!
#            P.colorbar(p)

            P.quiver(arrows.rs[:, 0], arrows.rs[:, 1], 
...
#        self.ratio_dat.append(d_trap / d_non)
    ...
    def update(self, arrows, box, t):
        ...
        if (self.ratio_flag) and (not self.iter_count % self.ratio_every):
