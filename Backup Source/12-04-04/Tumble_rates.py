...
class Tumble_rates(object):
    def __init__(self, num_rats, p_base):
        if num_rats < 0: raise Exception('Number of RATs < 0')
        if p_base < 0.0: raise Exception('Base tumble rate < 0.0')
...
class Tumble_rates_grad(Tumble_rates):
    ...
    def iterate_rates_main(self, rats, c):
        i = c.r_to_i(rats.r)
        v_dot_grad_c = np.sum(rats.v * utils.field_subset(c.get_grad(), i, 
                                                          rank=1), 1)
...
class Tumble_rates_mem(Tumble_rates):
    def __init__(self, num_rats, p_base, sense, t_max, dt):
        if t_max < 0.0: raise Exception('Motile memory < 0.0')
    ...
    def mem_kernel_find(self):
        ''' Calculate memory kernel and fudge to make quasi-integral exactly 
        zero ''' 
        A = 0.5
        ...
        self.K_dt[i_neg] *= np.abs(self.K_dt[i_pos].sum() / 
                                   self.K_dt[i_neg].sum())
        assert self.K_dt.sum() < ZERO_THRESH, 'Kernel not altered correctly'

    def iterate_rates_main(self, rats, c):
        i = c.r_to_i(rats.r)
        c_cur = utils.field_subset(c.a, i)
        numerics_box.rat_mem_integral(c_cur, self.c_mem, self.K_dt, 
                                      self.integrals)
