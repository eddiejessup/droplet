...
def grad(field, dx, walls):
    ...
    if field.ndim == 1:
        grad_1d(field, grad, dx, walls)
    elif field.ndim == 2:
        grad_2d(field, grad, dx, walls)
    elif field.ndim == 3:
        grad_3d(field, grad, dx, walls)
    else:
    ...
    for i_x in range(M_x):
        for i_y in range(M_y):
            if not walls[i_x, i_y]:
                ...
                grad[i_x, i_y, 0] = ((field[i_inc, i_y] - field[i_dec, i_y]) / 
                                     interval)
                ...
                grad[i_x, i_y, 1] = ((field[i_x, i_inc] - field[i_x, i_dec]) / 
                                     interval)
            else:
                grad[i_x, i_y, 0] = 0.0
                grad[i_x, i_y, 1] = 0.0
...
def grad_i(field, inds, dx, walls):
    ...
    if field.ndim == 1:
        grad_i_1d(field, inds, grad_i, dx, walls)
    elif field.ndim == 2:
        grad_i_2d(field, inds, grad_i, dx, walls)
    elif field.ndim == 3:
        grad_i_3d(field, grad_i, dx, walls)
    else:
...
def grad_i_3d(np.ndarray[np.float_t, ndim=3] field, 
    ...
    for i in range(inds.shape[0]):
        ...
        if not walls[i_x, i_y, i_z]:
            ...
            grad_i[i, 0] = ((field[i_inc, i_y, i_z] - field[i_dec, i_y, i_z]) / 
                            interval)
            ...
            grad_i[i, 1] = ((field[i_x, i_inc, i_z] - field[i_x, i_dec, i_z]) /
                            interval)
            ...
            grad_i[i, 2] = ((field[i_x, i_y, i_inc] - field[i_x, i_y, i_dec]) /
                            interval)
        else:
            grad[i, 0] = 0.0
            grad[i, 1] = 0.0
...
def laplace(field, dx, walls):
    ...
    if field.ndim == 1:
        laplace_1d(field, laplace, dx, walls)
    elif field.ndim == 2:
        laplace_2d(field, laplace, dx, walls)
    elif field.ndim == 3:
        laplace_3d(field, laplace, dx, walls)
    else:
