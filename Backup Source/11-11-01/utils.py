...
class Arrows_plot():
    def __init__(self, arrows, sim_flag):
        self.sim_flag = sim_flag
        self.lim = arrows.lattice.shape
        if self.sim_flag:
            fig = pylab.figure()
            pylab.ion()
            pylab.show()
    ...
    def update(self, arrows):
        if self.sim_flag:
#            self.sim.imshow(arrows.lattice.T, cmap=pylab.cm.binary, interpolation='none')
            self.sim.imshow(arrows.lattice.T, cmap=pylab.cm.binary, interpolation='nearest')

            # Scale to line up with maze plot
            self.rs = (arrows.rs + arrows.L / 2.0) * arrows.wall_density - 0.5
            self.vs = arrows.vs * arrows.wall_density
