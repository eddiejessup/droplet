...
f_0 = 0.0625
# attractant diffusion constant [l^2/t]
D_c = 20.0
# rate of conversion of food into attractant [l^2/t]
c_SOURCE_RATE = 0.1
...
if v_ALG == 't':
    ...
    LATTICE_RESOLUTION = int(np.ceil(L / DELTA_x_min))
    DELTA_x = L / float(LATTICE_RESOLUTION)

# Time-step validation
mins = [0.9 * DELTA_x, 0.8 * (DELTA_x ** 2.0) / (4.0 * D_c)]
necks = ["Walls (can't move >1 lattice point per step, decrease lattice resolution).", 
         "Diffusion (need stable diffusion algorithm, decrease lattice resolution or decrease diffusion constant)."]

DELTA_t_min_RAT = 0.1 * TUMBLE_TIME_BASE
if v_ALG == 't': 
    mins.append(DELTA_t_min_RAT)
    necks.append("Particle memory (need >= 10 memory points per run).")

i_mins_min = 0
for i_min in range(1, len(mins)):
    if mins[i_min] < mins[i_mins_min]:
        i_mins_min = i_min
...
DELTA_t = min(DELTA_t, mins[i_mins_min])

#raw_input()

# Find particles' maximum communication distance
if v_ALG == 'v' and COLLIDE_FLAG:
    r_COMM = max(COLLIDE_r, VICSEK_r)
elif v_ALG == 'v':
    r_COMM = VICSEK_r
elif COLLIDE_FLAG:
    r_COMM = COLLIDE_r
