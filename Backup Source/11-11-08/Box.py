...
class Box():
    ...
    def i_obstructed_find(self, rs):
        ...
        for i_arrow in range(i_lattice.shape[0]):
            try:
                if self.is_cell_wall(i_lattice[i_arrow]):
    ...
    def wall_backtrack(self, r_source, r_test, i_cell):
        ...
        self.r_b_1_v[0] = self.r_b_2_v[0] = sides[0]
        self.r_b_1_h[1] = self.r_b_2_h[1] = sides[1]
...
def main():
    ...
    r_a_1 = np.array([0.0, 0.0])
    r_a_2 = np.array([0.0, 0.5])
    r_b_1 = np.array([0.0, 0.0])
    r_b_2 = np.array([0.5, 0.0])
