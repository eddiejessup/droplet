...
class Arrows():
    def __init__(self, box, num_arrows, dt, 
                 rate_base, grad_sense, n_mem, mem_sense, 
                 onesided_flag, rates_alg, bc_alg):
        ...
        if rates_alg == 'const': self.rates_update = self._rates_update_const
        elif rates_alg == 'grad': self.rates_update = self._rates_update_grad
        elif rates_alg == 'mem': self.rates_update = self._rates_update_mem

        self._rs_initialise(box)
        self._vs_initialise(box)
        self._rates_initialise(n_mem, rates_alg)
    ...
    def _mem_kernel_find(self, n_mem):
        ...
        print('Kernel quasi-integral: %f' % (self.K.sum() * self.dt))
#        print('Kernel sensible quasi-integral: %f' % (self.K.sum() * self.dt * FOOD_0 * self.num_arrows * ATTRACT_RATE / (BREAKDOWN_RATE * WALL_RESOLUTION * LATTICE_RESOLUTION)))
#        P.plot(t_s * self.rate_base, self.K)
#        P.show()

    def _rates_initialise(self, n_mem, rates_alg):
