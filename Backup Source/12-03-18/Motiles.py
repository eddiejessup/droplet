...
class Motiles(object):
    def __init__(self, dt, num_motiles, v_base, walls, wall_alg, 
                 noise_flag, D_rot, 
        ...
        self.dt = dt
        self.dim = walls.dim
        self.num_motiles = num_motiles
        self.v_base = v_base
        self.walls = walls

        self.noise_flag = noise_flag
        if self.noise_flag:
            if D_rot < 0.0: raise Exception("Invalid rotational diffusion constant")
            self.D_rot = D_rot
            if self.dim == 1: 
                self.switching_prob = (2.0 * self.D_rot * self.dt) / np.square(np.pi)
            ...
            else: raise Exception("Rotational diffusion not supported in this dimension")

        self.r = np.empty([self.num_motiles, self.dim], dtype=np.float)
        self.r_end = self.r.copy()

        self.collide_flag = collide_flag
        if self.collide_flag:
            self.collide_R_sq = collide_R ** 2
            self.r_sep = np.empty([self.num_motiles, self.num_motiles, self.r.shape[-1]], dtype=np.float) 
            self.R_sep_sq = np.empty([self.num_motiles, self.num_motiles], dtype=np.float)
            self.r_sep_calced = True           
        else:
            self.r_sep_calced = False

        i_part = 0
        while i_part < self.num_motiles:
            self.r[i_part] = np.random.uniform(-self.walls.L / 2.0, +self.walls.L / 2.0, self.r.shape[-1])
        ...
        self.v = utils.point_pick_cart(self.dim, self.v_base, self.num_motiles) 
        self.v_temp = self.v.copy()

        self.wall_alg = wall_alg
        if self.wall_alg == 'align': self.wall_handle = self.wall_align
        elif self.wall_alg == 'specular': self.wall_handle = self.wall_specular
        elif self.wall_alg == 'bounceback': self.wall_handle = self.wall_bounceback
    ...
    def iterate_v_final(self):
        ...
        self.v[i_zeros] = utils.point_pick_cart(self.dim, self.v_base, len(i_zeros))

    def boundary_wrap(self):
        for i_dim in range(self.dim):
            i_wrap = np.where(np.abs(self.r_end[:, i_dim]) > self.walls.L / 2.0)[0]
            self.r_end[i_wrap, i_dim] -= np.sign(self.v[i_wrap, i_dim]) * self.walls.L
    ...
    def walls_avoid(self):
        ...
        delta_i[np.where(delta_i >= self.walls.size - 1)] -= self.walls.size
        delta_i[np.where(delta_i <= -(self.walls.size - 1))] += self.walls.size
        # If motile moved > 1 cell, bad news
        if len(np.where(delta_i > 1)[0]): raise Exception("v*dt > dx")
        # Find where motiles moved into a wall
        invalids = np.where(utils.field_subset(self.walls.a, motiles_i_end) == True)[0]
        for i_motile in invalids:
            for i_dim in np.where(delta_i[i_motile] != 0)[0]:
                ...
                offset[i_dim] = utils.wrap(self.walls.size, offset[i_dim])                
                if self.walls.a[tuple(offset)]:
                    ...
                    cell_r_start = self.walls.i_to_r(motiles_i_start[i_motile, i_dim])
                    self.r_end[i_motile, i_dim] = (cell_r_start + 
                                                   (self.walls.dx / 2.0 - BUFFER_SIZE) * 
    ...
    def r_sep_calc(self):
        numerics.r_sep_calc(self.r, self.r_sep, self.R_sep_sq, self.walls.L)
        self.r_sep_calced = True

    def collide(self):
        numerics.collide(self.v, self.r_sep, self.R_sep_sq, self.collide_R_sq)

    def add_noise_1d(self):
        utils.rotate_1d(self.v, self.switching_prob)

    def add_noise_2d(self):
        thetas = np.random.uniform(-self.eta_half, +self.eta_half, self.num_motiles)
        utils.rotate_2d(self.v, thetas)

    def add_noise_3d(self):
        ax = utils.point_pick_cart(self.dim, 1.0, self.num_motiles)
        thetas = np.random.uniform(-self.eta_half, +self.eta_half, self.num_motiles)
...
class Vicseks(Motiles):
    def __init__(self, dt, num_motiles, v_base, walls, wall_alg, 
                 ...
                 vicsek_R, sense):
        self.v_alg = 'v'
        super(Vicseks, self).__init__(dt, num_motiles, v_base, walls, wall_alg, 
                                      noise_flag, D_rot, 
                                      collide_flag, collide_R)
    ...
    def align(self):
        numerics.vicsek_align(self.v, self.v_temp, self.R_sep_sq, self.vicsek_R_sq)
...
class RATs(Motiles):
    def __init__(self, dt, num_motiles, v_base, walls, wall_alg, 
                 ...
                 collide_flag, collide_R, 
                 rates):
        super(RATs, self).__init__(dt, num_motiles, v_base, walls, wall_alg, 
                                   noise_flag, D_rot, 
                                   collide_flag, collide_R)
        self.v_alg = 't'
        self.rates = rates
    ...
    def tumble(self):
        dice_roll = np.random.uniform(0.0, 1.0, self.num_motiles)
        i_tumblers = np.where(dice_roll < (self.rates.p * self.dt))[0]
        self.v[i_tumblers] = utils.point_pick_cart(self.dim, self.v_base, len(i_tumblers.shape))