    ...
    def update_rs(self, box):
        ...
        for i_arrow in i_obstructed:
            ...
            while box.is_cell_wall(i_lattice[i_arrow]):
                rs_test[i_arrow], i_dim_hit, stick_flag = box.wall_backtrack(self.rs[i_arrow], rs_test[i_arrow], i_lattice[i_arrow])

                print('\tIteration: r_t: (%f, %f)\ti_l: (%i, %i)\tr_cell: (%f, %f)\ti_hit: %s' % (rs_test[i_arrow, 0], rs_test[i_arrow, 1],
                                                                                                  i_lattice[i_arrow, 0], i_lattice[i_arrow, 1],
                                                                                                  box.r_cell_find(i_lattice[i_arrow])[0], box.r_cell_find(i_lattice[i_arrow])[1],
                                                                                                  i_dim_hit))

                if stick_flag:
                    i_cell_blocker = i_lattice[i_arrow].copy()
                    i_cell_blocker[i_dim_hit] -= 1
                    if box.is_cell_wall(i_cell_blocker):
                        r_cell = box.r_cell_find(i_lattice[i_arrow])
                        rs_test[i_arrow] = r_cell + np.sign(self.rs[i_arrow] - r_cell) * box.d_half
                        
                        # Man this is hacky...
                        i_dim_hit = 1 - i_dim_hit
                    break
                else:
